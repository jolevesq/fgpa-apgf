...
function agolTemplate() {
    ...
    <section class="av-agol-login">
            ...
            <md-input-container class="md-block" flex-gt-xs>
                ...
                <input type="text" ng-model="self.agolPassword" av-focus required></input>
                <div class="av-error">{{ self.errorUser }}</div>
            </md-input-container>
            <md-input-container class="md-block" flex-gt-xs>
...
function agolController() {
    ...
    function downloadArcGIS() {
        ...
        api.getAgol().getToken(url, self.agolUsername, self.agolPassword).then(res => {
            api.getAgol().getItemId(url, self.agolId, res.token).then(res => parseAgol(res))
    ...
    function parseAgol(data) {
        ...
        for (let item of self.items) {
            item.value = (typeof data[item.agol]) !== 'undefined' ?
                deepAccessUsingString(data, item.agol) : 'sectionLabel';
        }
    }
    ...
    function importToModel() {
        ...
        const model = {
            'ui': api.getModel('ui'),
            'map': api.getModel('map')
        };

        // set values inside model
        for (let item of active) {
            deepAccessUsingString(model, item.model, item.value, item.model.split('.').length - 1)
        }
    }
